 ...
 */
function getGoogleMapsConfig(): GoogleMapsConfig {
...
export async function reverseGeocode(
    ...
    logger.info('Calling Google Geocoding API', { latitude, longitude });
    ...
    try {
        ...
        logger.error('Google Geocoding API call failed', { error, latitude, longitude });
        throw error;
    }
}
 ...
 */
export function parseGoogleAddress(geocodingResponse: GoogleGeocodingResponse): VtigerAddress {
    const result: VtigerAddress = {
        cf_995: '',  // Logradouro
        cf_763: '',  // Número
        cf_767: '',  // Bairro
    ...
    logger.info('Address parsed from Google Geocoding', { result });
...
export async function getAddressFromCoordinates(
    latitude: number,
    longitude: number
): Promise<VtigerAddress> {
    const geocodingResponse = await reverseGeocode(latitude, longitude);
